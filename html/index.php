            ...
            while ($row = mysqli_fetch_array($query)) {
                ...
                echo '<div class="card shadow-sm">
                        <div class="card-header">
                          <h4 class="my-0 font-weight-normal">'.$row['EventName'].'</h4>
                        </div>
                        <div class="card-body">
                          <h1 class="card-title pricing-card-title">Organized at '.$row['University'].'</h1>
						  '.$row['Where'].' at '$row['When']'
                          <a href="mailto:'.$row['CoordinatorEmail'].'" class="btn btn-lg btn-block btn-outline-primary">Contact '.$row['Coordinator'].'!</button>
                        </div>
                      </div>
                      ';
            }
